 ...
 * The class and constructor are public only because the RGT SectionTest
 * instantiates it directly.
 *
 * Origin: The starting point for this was Rev 4782 of AdHocParser.java (All RGT
 * 6/7 tests passed on this version.) Rev 4414 of AdHocParser didn't work; rev
 * 4782 changes apparently include fixes for Protex 6. So this is rev 4782 of
 * AdHocParser with the conditional logic removed by hand.
 *
 * @author sbillings
 * @param <T>
 *            the generic type
 */
public class AdHocParserProtex6<T extends HocElement> implements
	IAdHocParser<T> {
     ...
     */
    @Override
    public ArrayList<T> parseRows(Document doc,
	...
	try {
	    ...
	    Elements allReportSections = doc
		    .select(SELECTOR_RAW_ROWS_REPORTTABLE);
	    if (!nonConforming) {
	...
	for (Element row : rows) {
		...
		if (nextElement != null) {
		    ...
		    adHocRow.setPair(rowCell.text(),
			    ParserUtils.decode(nextElement.text()));
		    log.debug("Set Pair: " + rowCell.text() + " : "
			    + nextElement.text());
		}
		index++;
	    }
    ...
    private void parseConformingSection(HocElement adHocHeader,
	    ...
	    int targetSectionIndex, ArrayList<T> rowElements,
	    Integer counter, Elements allReportSections) throws Exception {
	...
	for (Element row : rows) {
		...
		if (adHocHeaderSize > 0) {
			...
			adHocRow.setPair(headerKey,
				ParserUtils.decode(rowCell.html().toString()));
			log.debug("    Set Pair: " + headerKey + " : "
				+ rowCell.html().toString());
		    } else {
			adHocRow.setPair(headerKey,
				ParserUtils.decode(rowCell.text()));
			log.debug("Set Pair in position: " + position + " -  "
				+ headerKey + " : " + rowCell.text());
		    }
		}
		position++;
	    } // row cell
     ...
     * of blank values. Protex occasionally includes a file in the
     * IdentifiedFiles section with all fields other than the filename blank.
     * These should be weeded out, and that's what this method does.
     *
     * @param adHocRow
     *            the ad hoc row
     * @param adHocHeader
     ...
     */
    private boolean rowContainsValidData(HocElement adHocRow,
	    HocElement adHocHeader) {
	...
	try {
	    /*
	     * Protex 6 reports: All sections: Headers are found in a table with
	     * class=reportTable.
	     *
	     ...
	     */
	    log.debug("Selector RawColums ReportTable = "
		    + SELECTOR_RAW_COLUMNS_REPORTTABLE);
	    ...
	    Elements verticalColumns = reportSection
		    .select(SELECTOR_TAG_TABLE_HEADER);
		...
		nonConforming = true; // Data labels are to the left of the
				      // values, rather than at the top of
				      // columns
	    }
     ...
     *             the exception
     */
    private T generateNewInstance(Class<T> hocElementClass)
	    throws Exception {
	T adHocRow = null;
